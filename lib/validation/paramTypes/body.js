...
var body = function(param, req, models, validationObj) {
  //var hasParamInBody = _.has(req.body, param.name);
  //var value = hasParamInBody ? req.body[param.name] : req.body;
  //var ret = helper.paramType(param, value, models, false);
  var value = _.has(req.body, param.name) ? req.body[param.name] : undefined;
  
  if (!_.some(ret, function(val) { return val.hasOwnProperty('error'); })) {
    if (validationObj.replaceValues) {
      req.body[param.name] = helper.getValue(param, ret);
      // if(hasParamInBody){
      //   req.body[param.name] = helper.getValue(param, ret);
      // }
      // else{
