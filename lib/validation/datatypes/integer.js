...
function validateInteger({schema, value, models, validationContext, validationSettings}) {
  ...
  if (validationSettings.allowNumbersToBeStrings) {
    ...
    isNumber = !(Array.isArray(value) || typeof value === 'boolean' || (isString(value) && value.trim() === ''));
  }
  ...
  if (!validationSettings.allowIntegerValuesWhichMayBeParsedIncorrectly) {
    if (valueAsNumber < Number.MIN_SAFE_INTEGER) {
      return validationError({
        code: errorCodes.INTEGER_IS_BELOW_MIN_SAFE_INTEGER_VALUE,
        propertyName: schema.name,
        value,
        validationContext,
        validationSettings
      });
    } else if (valueAsNumber > Number.MAX_SAFE_INTEGER) {
      return validationError({
        code: errorCodes.INTEGER_IS_ABOVE_MAX_SAFE_INTEGER_VALUE,
        propertyName: schema.name,
        value,
        validationContext,
        validationSettings
