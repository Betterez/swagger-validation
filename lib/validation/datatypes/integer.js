 ...
 */
function validateInt64({schema, value, models, validationContext, validationSettings}) {
  ...
  if (!isInt64(valueAsNumber) || !isNumber) {
 ...
 * or a value property that contains the value parsed successfully.
 */
function validateInt32({schema, value, models, validationContext, validationSettings}) {
  var isRequired = helper.isRequired({schema, value, validationContext, validationSettings});
  if (isRequired) {
    return isRequired;
  }
...
function validateInteger({schema, value, models, validationContext, validationSettings}) {
  ...
  switch (type) {
    case 'integer':
    case 'int32':
      return validateInt32({schema, value, models, validationContext, validationSettings});
    case 'int64':
      return validateInt64({schema, value, models, validationContext, validationSettings});
    default:
      if (validationSettings.throwErrorsWhenSchemaIsInvalid) {
        throw new Error(`Swagger schema is invalid: integer has unsupported format "${type}"`);
      } else {
        return validationError({
