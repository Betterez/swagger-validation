...
var validate = function({schema, value, models, validationContext, validationSettings}) {
  ...
  switch (type) {
    ...
    case 'date':
      if (validationSettings.allowStringsToHaveUnreliableDateFormat) {
        return validators.date({schema, value, models, validationContext, validationSettings});
      }

      if (validationSettings.throwErrorsWhenSchemaIsInvalid) {
        throw new Error(`Swagger schema is invalid: string has "date" format, but this format is unreliable because it parses dates without considering timezones.  Use the "date-time" format instead, or define a "pattern" to validate the format of the date string, and parse it yourself.`);
      } else {
        return validationError({
          code: errorCodes.DATE_FORMAT_UNSUPPORTED,
          validationContext,
          validationSettings
        });
      }
