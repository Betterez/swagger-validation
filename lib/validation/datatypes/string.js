...
var validateString = function(param, value) {
  ...
  if (param.enum && Array.isArray(param.enum) && !param.enum.some(function(val) { return val === value; })) {
  ...
  if (param.hasOwnProperty('pattern') && param.pattern !== null && param.pattern !== undefined) {
    try {
      if (!helper.isString(param.pattern)) {
  ...
  if (param.maxLength && Number.isInteger(param.maxLength) && value.length > param.maxLength) {
    return helper.errorReturn("%s requires a max length of %s", [param.name, param.maxLength]);
  }

  if (param.minLength && Number.isInteger(param.minLength) && value.length < param.minLength) {
    return helper.errorReturn("%s requires a min length of %s", [param.name, param.minLength]);
  }
