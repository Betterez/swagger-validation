 ...
 */
function validateDate({schema, value, models, validationContext, validationSettings}) {
  ...
  var pattern = schema.pattern || moment.ISO_8601;
  var patternDisplay = schema.pattern || 'for moment.ISO 8601';
  var date = moment(value, pattern);

  if (!date.isValid()) {
    return validationError({
      message: `${schema.name} is not valid based on the pattern ${patternDisplay}`,
