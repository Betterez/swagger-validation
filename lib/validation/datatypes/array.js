...
const {isPlainObject, mapByProperty, uniqBy} = require('../native-utils');
 ...
 * This strictly converts everything to strings (using JSON.stringify()) and compares those strings for uniqueness.
 *
 ...
 */
function validateArray({schema, value, models, validationContext, validationSettings, validationLogs}) {
  ...
  value.forEach((arrayItem, index) => {
    ...
    const isObject = isPlainObject(arrayItem);
    ...
    if (validationResults.some((val) => val.hasOwnProperty('error'))) {
      errors = errors.concat(validationResults);
    }
    else {
      values = values.concat(mapByProperty(validationResults, 'value'));
    }
  });

  if (errors.length === 0 && schema.uniqueItems) {
    var unique = uniqBy(values, JSON.stringify);
  ...
  let transformedArray = values;

  if (validationSettings.removeNullValuesFromArrays) {
    transformedArray = _removeNullValuesFromArray({schema, value, models, validationContext, validationSettings, validationLogs});
  }

  return successReturn(transformedArray);
}

function _removeNullValuesFromArray({schema, value, models, validationContext, validationSettings, validationLogs}) {
  const {schema: schemaForArrayItems} = resolveSchema({
    schema: schema.items,
    models,
    validationContext,
    validationSettings
  });
  const itemsAreExplicitlyNullable = schemaForArrayItems.nullable === true;

  if (itemsAreExplicitlyNullable) {
    return value;
  }

  return value.filter((arrayItem, index) => {
    if (arrayItem === null) {
      validationLogs.logNullValueRemovedFromArray(validationContext.descendIntoProperty(index));
    }
