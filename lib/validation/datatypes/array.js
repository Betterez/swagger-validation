 ...
 */
function validateArray(schema, value, models, validationContext) {
  ...
  if (schema.minItems && !isNaN(schema.minItems) && value.length < schema.minItems) {
    return validationError({
      message: `${schema.name} should have at least ${schema.minItems} item(s)`,
      context: validationContext
    });
  }
  ...
  if (schema.maxItems && !isNaN(schema.maxItems) && value.length > schema.maxItems) {
    return validationError({
      message: `${schema.name} should have at most ${schema.maxItems} item(s)`,
      context: validationContext
    });
  }
  _.forEach(value, (value, index) => {
    ...
    newParam.name = (isObject || isNull) ? paramType : value;
    const newValidationContext = validationContext.descendIntoProperty(index);
    var paramReturn = validateParameter(newParam, value, models, newValidationContext);

    if (_.some(paramReturn, function(val) { return val.hasOwnProperty('error'); })) {
      errors = errors.concat(paramReturn);
    }
    else {
      values = values.concat(_.map(paramReturn, 'value'));
    }
  });
  ...
  if (errors.length === 0 && schema.uniqueItems) {
    ...
    if (unique.length !== values.length) {
      return validationError({
        message: `${schema.name} is not unique. This may lead to an unintended loss of data`,
        context: validationContext
      });
    }
  }
