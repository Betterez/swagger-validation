...
var validate = function(param, value, models) {
  ...
  if (param.minItems && !isNaN(param.minItems) && value.length < param.minItems) {
    return helper.errorReturn("%s should have at least %s item(s)", [param.name, param.minItems]);
  }

  if (param.maxItems && !isNaN(param.maxItems) && value.length > param.maxItems) {
    return helper.errorReturn("%s should have at most %s item(s)", [param.name, param.maxItems]);
  }
  _.forEach(value, function(value) {
    ...
    const paramType = param.items.$ref || param.items.type;
