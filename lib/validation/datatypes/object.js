 ...
 */
function validateObject(schema, value, models, validationContext) {
  // This is imported late due to a circular dependency.
  const {validateParameter} = require('../parameter');

  const isRequired = helper.isRequired(schema, value, validationContext);
  if (isRequired) {
    return isRequired;
  }
  ...
  errors.push(..._validateAdditionalProperties(schema.name, model, value, validationContext));
  ...
  Object.entries(model.properties ?? {}).forEach(([propertyName, propertySchema]) => {
    ...
    const newValidationContext = validationContext.descendIntoProperty(propertyName);
    const validationResults = validateParameter(parameterSchema, value[propertyName], models, newValidationContext);
...
function _validateAdditionalProperties(parentParameterName, model, value, validationContext) {
  if (model.additionalProperties === false) {
    ...
    if (disallowedProperties.length > 0) {
      return validationError({
        message: `${parentParameterName} contains invalid properties: ${disallowedProperties.join(", ")}`,
        context: validationContext
      });
    }
  }
 ...
 */
function validate(schema, value, models, validationContext) {
  ...
  return schemaIsKnown
    ? validateObject(schema, value, models, validationContext)
    : validationError({
