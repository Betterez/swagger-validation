...
function validateObject({schema, value, models, validationContext, validationSettings, validationLogs}) {
  ...
  if (validationSettings.removeNullValuesFromObjects) {
    _removeNullValues({schema, value, models, validationContext, validationSettings, validationLogs});
  }
...
function _removeNullValues({schema, value, models, validationContext, validationSettings, validationLogs}) {
  for (const [propertyName, propertyValue] of Object.entries(value)) {
    let isPropertyExplicitlyNullable = false;

    if (schema.properties?.[propertyName]) {
      const {schema: schemaForProperty, validationContext: validationContextForProperty} = resolveSchema({
