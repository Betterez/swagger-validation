...
const errorCodes = {
  ...
  INTEGER_IS_BELOW_MIN_SAFE_INTEGER_VALUE: 'INTEGER_IS_BELOW_MIN_SAFE_INTEGER_VALUE',
  INTEGER_IS_ABOVE_MAX_SAFE_INTEGER_VALUE: 'INTEGER_IS_ABOVE_MAX_SAFE_INTEGER_VALUE',
  ...
  DATE_FORMAT_UNSUPPORTED: 'DATE_FORMAT_UNSUPPORTED',
...
function getLegacyErrorMessage(errorCode, stringTemplateParams) {
  ...
  switch (errorCode) {
    ...
    case errorCodes.INTEGER_IS_BELOW_MIN_SAFE_INTEGER_VALUE:
      return `${propertyName} is below the minimum possible value for an integer`;
    case errorCodes.INTEGER_IS_ABOVE_MAX_SAFE_INTEGER_VALUE:
      return `${propertyName} is above the maximum possible value for an integer`;
    ...
    case errorCodes.DATE_FORMAT_UNSUPPORTED:
      return `Swagger schema is invalid: string has "date" format, but this format is unreliable because it parses dates without considering timezones.  Use the "date-time" format instead, or define a "pattern" to validate the format of the date string, and parse it yourself.`;
...
function getErrorMessage(errorCode, stringTemplateParams) {
  ...
  switch (errorCode) {
    ...
    case errorCodes.INTEGER_IS_BELOW_MIN_SAFE_INTEGER_VALUE:
      return `${validationContext.formatDataSource()} is invalid: ${validationContext.formatDataPath()} has a value of ${value} which is below the minimum possible value for an integer (${Number.MIN_SAFE_INTEGER})`;
    case errorCodes.INTEGER_IS_ABOVE_MAX_SAFE_INTEGER_VALUE:
      return `${validationContext.formatDataSource()} is invalid: ${validationContext.formatDataPath()} has a value of ${value} which is above the maximum possible value for an integer (${Number.MAX_SAFE_INTEGER})`;
    case errorCodes.NUMBER_IS_BELOW_MINIMUM_VALUE:
...
function validationError(props) {
  const {code: errorCode, validationContext, validationSettings} = props;

  if (!errorCode) {
    throw new Error('A validation error must have a "code".');
  }
