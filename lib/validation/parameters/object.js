...
var validateObject = function(param, value, models) {
  ...
  var func = function(param, value) {
    var model = models[param.type || param.$ref];
      ...
      _.forOwn(model.properties, function(val, key) {
        ...
        if (value.hasOwnProperty(key)) {
          ...
          if (_.some(paramReturn, function(val) { return val.hasOwnProperty('error'); })) {
            var failed = _.filter(paramReturn, function(val) { return val.hasOwnProperty('error'); });
            failed.forEach(function(f) {
              f.path = f.path ? f.path + " - " + param.type : "" + param.type;
              f.failedValue = f.failedValue ? f.failedValue : value[key];
            });
            errors = errors.concat(paramReturn);
          } else {
            value[key] = _.chain(paramReturn).map('value').first().value();
          }
        }
        else if (isRequired) {
    ...
    _.forEach(models, function(model) {
      if (model.hasOwnProperty('subTypes')) {
        if (_.includes(model.subTypes, param.type)) {
          param.name = model.id;
          param.type = model.id;
          func(param, value);
        }
...
var validate = function(param, value, models) {
  return models.hasOwnProperty(param.type || param.$ref) ? validateObject(param, value, models) : helper.errorReturn('Unknown param type %s', param.type);
};
