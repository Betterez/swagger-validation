...
function validateObject(param, value, models) {
  ...
  function _validate(param, value) {
    ...
    errors.push(..._validateAdditionalProperties(model, value));
...
function _validateAdditionalProperties(model, value) {
  if (model.additionalProperties === false) {
    ...
    if (disallowedProperties.length > 0) {
      return helper.errorReturn(`Properties are not allowed: ${disallowedProperties.join(", ")}`);
    }
  }
