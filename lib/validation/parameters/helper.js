...
function isRequired(param, value) {
  if (value === null && param.nullable === false) {
    return errorReturn(`${param.name} cannot be null`);
  }

  const allowsEmptyString = !(param.pattern || param.minLength);

  if (_.isUndefined(value) || _.isNull(value) || (value === '' && allowsEmptyString)) {
    var isParamRequired = param.required && !param.defaultValue;
    return isParamRequired
      ? errorReturn(`${param.name} is required`)
