 ...
 */
function validateRequestPath({schema, req, models, validationContext, validationSettings}) {
  ...
  const validationResults = validateParameter({schema: schemaForPath, isRequired: schema.required, value, models, validationContext, validationSettings});

  if (!validationResults.some((result) => result.hasOwnProperty('error'))) {
    if (validationSettings.replaceValues) {
      req.params[schema.name] = helper.getTransformedValue(schema, validationResults);
    }
  }
