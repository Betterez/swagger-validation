 ...
 */
function validateRequestHeader({schema, req, models, validationContext, validationSettings}) {
  ...
  const validationResult = validateParameter({schema: schemaForValue, value, models, validationContext, validationSettings});

  if (!validationResult.some((result) => result.hasOwnProperty('error'))) {
    if (validationSettings.replaceValues) {
      req.header[schema.name] = helper.getTransformedValue(schema, validationResult);
    }
  }
