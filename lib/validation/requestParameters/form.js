 ...
 */
function validateRequestForm({schema, req, models, validationContext, validationSettings}) {
  ...
  if (!validationResult.some((result) => result.hasOwnProperty('error'))) {
    if (validationSettings.replaceValues) {
      req.form[schema.name] = helper.getTransformedValue(schema, validationResult);
    }
  }
