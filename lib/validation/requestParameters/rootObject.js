 ...
 */
function validateRootObject(param, key, obj, models, validationSettings) {
  if (!param.name) {
    param.name = key;
  }
  ...
  const ret = validateParameter(param, value, models);

  if (!_.some(ret, function(val) { return val.hasOwnProperty('error'); })) {
    if (validationSettings.replaceValues) {
       if (hasParam) {
         obj[param.name] = helper.getTransformedValue(param, ret);
       }
    }
  }
  return ret;
