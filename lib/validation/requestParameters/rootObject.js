...
const {hasPath, isEmpty} = require('../native-utils');
 ...
 */
function validateRootObject({schema, key, value, models, validationSettings, validationLogs}) {
  if (!schema.name) {
    schema.name = key;
  }

  const hasParam = hasPath(value, schema.name);
  const parameterValue = hasParam ? value[schema.name] : isEmpty(value) ? undefined : value;
  ...
  if (!validationResult.some((val) => val.hasOwnProperty('error'))) {
