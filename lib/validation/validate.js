...
const {validateRequestContent} = require('./requestContent');
...
function getValidationSettings(validationSettings = {}) {
  return {
    enabled: validationSettings.hasOwnProperty('enabled') ? validationSettings.enabled : true,
 ...
 */
function validate(spec, req, models) {
  ...
  const validationResults = _.flatMap(requestSchema, (parameterSpec, key) => {
    return validateRequestContent(parameterSpec, req, _models, validationSettings, key);
  });
