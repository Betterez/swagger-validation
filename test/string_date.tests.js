...
describe('string - date', function() {
  ...
  it('should validate', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date'), value, models, validationContext);
    helper.assertValidationPassed(ret, [expected]);
  });

  it('a date should always validate', function() {
    var expected = moment('2014-01-01', 'YYYY-MM-DD').toDate();
    var ret = validateParameter(helper.makeStringParam('string', false, 'date'), expected, models, validationContext);
    helper.assertValidationPassed(ret, [expected]);
  });
  ...
  it('should validate with custom pattern', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date', 'M/D/YYYY'), value, models, validationContext);
    helper.assertValidationPassed(ret, [expected]);
  });
  ...
  it('should not validate with random string', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date'), value, models, validationContext);
    helper.assertValidationFailed(ret, ["testParam is not valid based on the pattern for moment.ISO 8601"]);
  });

  it('should not validate with required field null', function() {
    var value = null;
    var ret = validateParameter(helper.makeStringParam('string', true, 'date'), value, models, validationContext);
    helper.assertValidationFailed(ret, ["testParam is required"]);
  });

  it('should not validate with required field undefined', function() {
    var ret = validateParameter(helper.makeStringParam('string', true, 'date'), undefined, models, validationContext);
    helper.assertValidationFailed(ret, ["testParam is required"]);
  });
