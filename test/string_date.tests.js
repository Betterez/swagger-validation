...
describe('string - date', function() {
  it('should validate', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date'), value);
    helper.validateSuccess(ret, 1, [expected]);
  });

  it('a date should always validate', function() {
    var expected = moment('2014-01-01', 'YYYY-MM-DD').toDate();
    var ret = validateParameter(helper.makeStringParam('string', false, 'date'), expected);
    helper.validateSuccess(ret, 1, [expected]);
  });
  ...
  it('should validate with custom pattern', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date', 'M/D/YYYY'), value);
    helper.validateSuccess(ret, 1, [expected]);
  });
  ...
  it('should not validate with required field null', function() {
    var value = null;
    var ret = validateParameter(helper.makeStringParam('string', true, 'date'), value);
    helper.validateError(ret, 1, ["testParam is required"]);
  });

  it('should not validate with required field undefined', function() {
    var ret = validateParameter(helper.makeStringParam('string', true, 'date'), undefined);
    helper.validateError(ret, 1, ["testParam is required"]);
  });
