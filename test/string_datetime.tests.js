...
describe('string - datetime', function() {
  it('should validate', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date-time'), value);
    helper.validateSuccess(ret, 1, [expected]);
  });

  it('a date should always validate', function() {
    var expected = moment('2014-01-01T17:00:00-0700').toDate();
    var ret = validateParameter(helper.makeStringParam('string', false, 'date-time'), expected);
    helper.validateSuccess(ret, 1, [expected]);
  });
  ...
  it('should validate with custom pattern', function() {
    ...
    var ret = validateParameter(helper.makeStringParam('string', false, 'date-time', 'M/D/YYYY h:mmA'), value);
    helper.validateSuccess(ret, 1, [expected]);
  });
  ...
  it('should not validate with required field null', function() {
    var value = null;
    var ret = validateParameter(helper.makeStringParam('string', true, 'date-time'), value);
    helper.validateError(ret, 1, ["testParam is required"]);
  });

  it('should not validate with required field undefined', function() {
    var ret = validateParameter(helper.makeStringParam('string', true, 'date-time'), undefined);
    helper.validateError(ret, 1, ["testParam is required"]);
  });
